...
/* ============================================================
   ✅ RAILWAY HEALTHCHECK (CRITIQUE)
   → DOIT démarrer AVANT la connexion Discord (sinon SIGTERM loop)
============================================================ */

const PORT = process.env.PORT || 3000;

let healthServer = http.createServer((req, res) => {
...
client.on('guildMemberRemove', async member => {
  if (member.guild.id === SUPPORT_GUILD_ID) await updateMemberCounter(SUPPORT_GUILD_ID);
  if (member.guild.id === IG_GUILD_ID) await updateMemberCounter(IG_GUILD_ID);
});
