...
// Champs enregistrés dans servers.json (guildConfig):
// - disposChannelId (📅) [REQUIS]
// - staffReportsChannelId (📊) [REQUIS]
// - staffRoleId (🛡️) [REQUIS]
// - playerRoleId (👟) [REQUIS]
// - trialRoleId (🧪) [opt]
// - pseudo.scanChannelId (🎮) [opt]
// - automations.enabled (🤖 ON/OFF)
//
...
function isOwner(i, scope) {
  return i?.customId?.endsWith(scope) && i.user?.id && scope.endsWith(i.user.id);
}
...
module.exports = {
  ...
  async execute(interaction) {
    try {
      ...
      // Message 1 (salons + save/reset) => 4 rows
      ...
      // Message 2 (rôles + auto/cancel) => 5 rows
      ...
      col1.on("collect", async (i) => {
        try {
          if (!isOwner(i, scope) || i.user.id !== interaction.user.id) {
          ...
          if (i.isButton()) {
            ...
            if (i.customId === CID.save) {
              ...
              const patch = {
                ...
                pseudo: {
                  ...(saved.pseudo || {}),
                  scanChannelId: draft.pseudoScanChannelId,
                },
              ...
              await i.update({ content: ICON.save, embeds: [buildEmbed(guild, draft, enabled)], components: [] });
