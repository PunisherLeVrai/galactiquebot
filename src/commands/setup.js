...
function normalizeWarningRoleIds(input) {
  ...
  for (let index = 0; index < 3; index++) {
    const id = isSnowflake(source[index])
      ? String(source[index])
      : null;
...
function opensModal(customId) {
  return /setup:(modal|btn:(confirmSave|openMinute|idsA|idsB))/.test(
    String(customId || "")
  );
}
  ...
  return new EmbedBuilder()
    ...
    .setDescription(
      `${
        ...
        "Format pseudo : **POSTE / RÔLE / PSEUDO** — en MAJUSCULES."
    )
    .addFields(
      {
        name: "Salons",
        value: [
          ...
          `📊 Rapports staff : ${fmtChannel(
            draft.staffReportsChannelId
          )}`,
          `🗓️ Check Dispo : ${fmtChannel(
...
module.exports.execute =
  async function execute(interaction) {
    ...
    for (const [key, prefix] of Object.entries({
      ...
      dispos: "ch:dispos",
      staffReports: "ch:staff",
      pseudoScan: "ch:pseudo",
      checkDispo: "ch:check",
      ...
      warning3: "role:warning3",

      idsA: "btn:idsA",
      idsB: "btn:idsB",
      idsClear: "btn:idsClear",

      idsAModal: "modal:idsA",
      idsBModal: "modal:idsB",

      autoTab: "auto:tab",
      autoGlobal: "btn:autoGlobal",
      ...
      if (i.isModalSubmit?.()) {
        ...
        if (
          ...
          if (
            ...
            return i.reply({
              content:
                "⚠️ Configure au minimum Avertissement 1.",
              flags:
                MessageFlags.Ephemeral,
            });
          }
          ...
          if (
            ...
            return i.reply({
              content:
                "⚠️ Configure les rôles dans l’ordre 1, 2, 3.",
              flags:
                MessageFlags.Ephemeral,
            });
          }
          ...
          if (
            ...
            return i.reply({
              content:
                "⚠️ Configure au moins un horaire CheckDispo.",
              flags:
                MessageFlags.Ephemeral,
            });
          }
      ...
      if (
        ...
        return i.showModal(
          idsModal(
            ...
            ["Sam", "Dim"],
            draft.dispoMessageIds,
            "IDs Samedi et Dimanche"
          )
        );
      }

      if (
        i.customId === CID.cancel
      ) {
        return end();
      }

      if (
        i.customId === CID.preview
      ) {
        return refresh();
      }
