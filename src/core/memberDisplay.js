...
function resolveStaffRole(member, cfg) {
  ...
  const staffRoles = member.roles?.cache?.filter(
    (role) =>
      role &&
      staffRoleIds.includes(String(role.id))
  );
...
function resolveConfiguredDisplayRole(member, cfg) {
  const roleId = cfg?.displayRoleId
    ? String(cfg.displayRoleId)
    : null;
...
function resolveMainRole(member, cfg) {
  return (
    resolveStaffRole(member, cfg) ||
    resolveConfiguredDisplayRole(member, cfg)
  );
}
...
function fitNickname(postsText, roleText, pseudoText) {
  ...
  const maxPseudoLength = Math.max(
    1,
    32 - prefix.length
  );

  const pseudo = pseudoText.slice(
    0,
    maxPseudoLength
  );
